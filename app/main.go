...
var state run.CleanupPricelistHistoriesState
...
func init() {
	...
	logging.WithFields(logrus.Fields{
		...
		"port":         port,
	}).Info("Producing cleanup-pricelist-histories state")

	state, err = run.NewCleanupPricelistHistoriesState(run.CleanupPricelistHistoriesStateConfig{ProjectId: projectId})
	if err != nil {
